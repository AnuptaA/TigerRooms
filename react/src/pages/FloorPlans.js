...
const FloorPlans = () => {
    ...
    return (
        <div>
            <h1 className='results-page-title'>Showing results for all floor plans</h1>
...
const AvailabilityTable = ({availabilityInfo}) => {
    ...
    const maxFloors = availabilityInfo.length > 0 ? Math.max(...availabilityInfo.map(info => info.floors.length)): 0;

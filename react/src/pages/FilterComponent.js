...
const FilterComponent = () => {
  ...
  const handleSubmit = () => {
    ...
    const formattedHall = hall.replace(' Hall', '').toLowerCase().replace(/\s+/g, '-'); // Remove "Hall" and replace spaces with hyphens
    ...
    <div className="filter-container">
      ...
      <div className="dropdown-container">
        <div className="dropdown-group">
          ...
          >
            <option value="" className="placeholder-option">Select Residential College</option>
            {residentialColleges.map((college, idx) => (
              <option key={idx} value={college}>{college}</option>
            ))}
          </select>
          {error && <p style={{ color: 'red', fontSize: '0.9em' }}>Please select a Residential College.</p>}
        </div>

        <div className="dropdown-group">
          <label htmlFor="hall">Hall</label>
          <select id="hall" value={hall} onChange={(e) => setHall(e.target.value)}>
            <option value="" className="placeholder-option">Select Hall</option>
            {whitmanHalls.map((hallOption, idx) => (
              <option key={idx} value={hallOption}>{hallOption}</option>
            ))}
          </select>
        </div>
        ...
        <div className="dropdown-group">
          ...
          <select id="floor" value={floor} onChange={(e) => setFloor(e.target.value)}>
            <option value="" className="placeholder-option">Select Floor</option>
        ...
        <div className="dropdown-group">
          ...
          <select id="occupancy" value={occupancy} onChange={(e) => setOccupancy(e.target.value)}>
            <option value="" className="placeholder-option">Select Occupancy</option>
      ...
      <div className="button-container">
        <button className='submit-button' onClick={handleSubmit}><strong>SUBMIT</strong></button>
        <button className='reset-button' onClick={handleResetFilters} style={{ marginLeft: '10px' }}><strong>RESET</strong></button>
      </div>
    </div>
  );
};
